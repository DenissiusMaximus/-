<div class="max-w-lg">
    <h1 class="text-xl font-semibold text-gray-800 mb-4">Редагувати ціль #<?= $aim->id ?></h1>

    <form method="POST" action="<?= url('aims/update/' . $aim->id) ?>" class="bg-white border border-gray-200 rounded p-6 space-y-4">

        <div>
            <label class="block text-sm text-gray-600 mb-1">Назва</label>
            <input type="text" name="name" value="<?= htmlspecialchars($aim->name) ?>" required
        ...
        <div>
            ...
            <input type="number" step="0.01" name="target_amount" value="<?= $aim->target_amount ?>" required
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
        </div>

        <div>
            <label class="block text-sm text-gray-600 mb-1">Вже зібрано (грн)</label>
            <input type="number" step="0.01" name="current_amount" value="<?= $aim->current_amount ?>"
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
        </div>
