<div class="max-w-lg">
    <h1 class="text-xl font-semibold text-gray-800 mb-4">Редагувати транзакцію #<?= $transaction->id ?></h1>

    <form method="POST" action="<?= url('transactions/update/' . $transaction->id) ?>" class="bg-white border border-gray-200 rounded p-6 space-y-4">
        ...
        <div>
            ...
            <select name="source_id" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
                ...
                <option value="<?= $src->id ?>" <?= $src->id === $transaction->source_id ? 'selected' : '' ?>>
        ...
        <div>
            ...
            <input type="text" name="comment" value="<?= htmlspecialchars($transaction->comment ?? '') ?>"
                class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-blue-500">
        </div>
